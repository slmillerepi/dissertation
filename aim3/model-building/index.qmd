...
$$
\begin{aligned}
Y_{i,s,t,m} &= \beta_0 + \sum_{j=2}^p\beta_jx_{j,i} + (\beta_1 + b_m^{(x_1m)} + b_t^{(x_1t)})x_{1,i} + b_s^{(s)} +b_t^{(t)} + b_m^{(m)} + \epsilon_{i,s,t,m}\\
\end{aligned}
$$ 
...
$b_m^{(m)} \sim \mathcal{N}(0, \sigma^2_m)$|Random intercept for antigenic distance method $m$
$b_m^{(x_1m)} \sim \mathcal{N}(0, \sigma^2_{x_1m})$|Random slope for antigenic distance predictor $x_1$, varying by method $m$
$b_t^{(x_1t)} \sim \mathcal{N}(0, \sigma^2_{x_1t})$|Random slope for antigenic distance predictor $x_1$, varying by subtype $t$
$\epsilon_{i,s,t,m} \sim \mathcal{N}(0, \sigma^2_\epsilon)$|Residual
